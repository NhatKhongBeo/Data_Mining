...
class NeuralNetwork:
    ...
    def backward_propagation_with_regularization(self,AL,Y,caches,lambd):
        ...
        for l in reversed(range(L-1)):
            ...
            dA_prev_temp, dW_temp, db_temp = self.linear_activation_backward(grads["dA" + str(l + 1)], current_cache, activation = "relu")
            grads["dA" + str(l)] = dA_prev_temp
            grads["dW" + str(l + 1)] = dW_temp + (lambd/m)*self.parameters["W" + str(l + 1)]
            grads["db" + str(l + 1)] = db_temp
    ...
    def backward_propagation_with_dropout(self,AL,Y,caches,keep_prob):
        ...
        for l in reversed(range(L-1)):
            ...
            dA = dA * D
            dA = dA / keep_prob
            dA_prev_temp, dW_temp, db_temp = self.linear_activation_backward(dA, cache_tmp, activation = "relu")
            grads["dA" + str(l)] = dA_prev_temp
            grads["dW" + str(l + 1)] = dW_temp
            grads["db" + str(l + 1)] = db_temp
    ...
    def fit(self,X,Y,num_iterations,learning_rate=0.01,lambd=0,keep_prob=1,print_cost=False):
        np.random.seed(1)
        costs = []
        for i in range(0,num_iterations):
            if keep_prob == 1:
                ...
                AL,caches = self.forward_propagation_with_dropout(X,self.parameters,keep_prob)
    ...
    def train(self,X,Y,num_epochs,learning_rate=0.01,batch_size=32,lambd=0,keep_prob=1, print_cost=False):
        np.random.seed(1)
        costs = []
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(num_interations):
                ...
                if keep_prob == 1:
                    ...
                    AL,caches = self.forward_propagation_with_dropout(X_batch,self.parameters,keep_prob)
                
                if lambd == 0:
                    cost = self.compute_cost(AL,Y_batch)
                else:
                    cost = self.compute_cost_with_regularization(AL,Y_batch,self.parameters,lambd)
                ...
                if lambd == 0 and keep_prob == 1:
                    ...
                    grads = self.backward_propagation_with_dropout(AL,Y_batch,caches,keep_prob)
                self.parameters = self.update_parameters(self.parameters,grads,learning_rate)
                epoch_cost += cost/num_interations
